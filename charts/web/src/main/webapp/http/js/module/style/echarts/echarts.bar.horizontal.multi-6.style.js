...
define(function(require, exports, module) {
	...
	var templateHtml = require("/http/js/module/style/echarts/echarts.bar.horizontal.multi-6.html.tpl");
    ...
    var EChartBarVerticalMultiStyle = (function(){
		...
		var eventBind = function(hanlder,callback){
			if(hanlder){
				...
				if(chartObj && objConfig){
					...
					//x轴数值可调
					$("#chart-xaxis-change").change(function(){
						var text = $(this).val();
		...
		var initControlStyle = function(){
			...
			var xAxismin = xAxisStyle.xAxis[0].min;
			if(xAxismin!=-1 ){
				$("#chart-xaxis-change").attr("value",xAxismin);
				var $xAxismin =  $("#chart-xaxis-change").parent("div");
				//保证控件刷新
				$xAxismin.html($xAxismin.html());
			}//end if(xAxisStyleLineColor != null)
