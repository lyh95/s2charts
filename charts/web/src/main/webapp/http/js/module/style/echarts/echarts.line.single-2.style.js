...
define(function(require, exports, module) {
	...
	var templateHtml = require("/http/js/module/style/echarts/echarts.line.single-2.html.tpl");
	...
	var EChartLineSingleStyle = (function(){
		...
		var initControlStyle = function(){
			...
			if(series && series[0]){
				//折线是否为平滑曲线
				var smooth = series[0].smooth;
				if(smooth && smooth === true){
					$("#chart-show-smooth-check").attr("checked",true);
				}else{
					$("#chart-show-smooth-check").attr("checked",false);
				}

				//折线宽度
				var width= $.extend(true,{},{series:[
					{
						"itemStyle": {
							"normal": {
								"lineStyle": {
				...
				var w = width.series[0].itemStyle.normal.lineStyle.width;
				if(w > -1){
				...
				//是否显示标签
				var textStyle= $.extend(true,{},{series:[
					{
						"itemStyle": {
							"normal": {
								"label": {
				...
				if(textStyle.series[0].itemStyle.normal.label.show === false){
				...
				var textStylePosition = textStyle.series[0].itemStyle.normal.label.position;
				...
				var textStyleColor = textStyle.series[0].itemStyle.normal.label.textStyle.color;
				...
				var textStyleFontSize = textStyle.series[0].itemStyle.normal.label.textStyle.fontSize;
			...
			if(color && color[0]){
				$("#chart-line-color-select").attr("value",color[0]);
