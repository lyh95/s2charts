...
define(function(require, exports, module) {
	...
	var templateHtml = require("/http/js/module/style/echarts/echarts.pie.bar.html.tpl");
	var inputHtml = "<input type=\"color\" class=\"form-control\" style=\"width:45px;\" value=\"#26C0C0\" />";
	
    /**
     *
     */
    var EChartPieBarStyle = (function(){
        var _isInit = false;
    	var _style = {};
    	var _container = {};
        ...
        var reBindColorSelect = function(chartObj,objConfig){
        	...
        	//添加饼的颜色
	    	var currentColors = $.extend([],chartObj.pieSingle.currentColors || []);
	    	var colorsLength = currentColors.length;
	    	
        	//若已经修改过配色，直接显示配色的
        	if(_style.pie && _style.pie.color && _style.pie.color.length > 0){
        		//表示没有新增数据
        		if(_style.pie.color.length >= colorsLength){
        			//修改数据长度
        			_style.pie.color.length = colorsLength;
        			for(var c=0,cLen=colorsLength;c<cLen;c++){
        				//添加新的颜色
        	    		$(inputHtml).attr("id","chart-pie-color-select-"+c).val(_style.pie.color[c])
        	    			.appendTo($("#chart-pie-color-container"));
            		}
        		}else{
        			//有新增的数据
        			for(var d=0,dLen=_style.pie.color.length;d<dLen;d++){
        				//添加新的颜色
        	    		$(inputHtml).attr("id","chart-pie-color-select-"+d).val(_style.pie.color[d])
        	    			.appendTo($("#chart-pie-color-container"));
            		}
        			
        			var index = _style.pie.color.length;
        	...
        	}//end if(_style.pie && _style.pie.color && _style.pie.color.length > 0) else
		...
		var eventBind = function(hanlder,callback){
			if(hanlder){
				...
				if(chartObj && objConfig){
					...
					//是否显示图列(饼)
					$("#chart-show-pie-legend-check").change(function(){
						var isShow = $(this).is(":checked");
					...
					$("#chart-show-pie-label-check").change(function(){
						...
						$("input[name='chart-pie-font-position-radio']").attr("disabled",!isShow);
					...
					//选择字体位置
					$("input[name='chart-pie-font-position-radio']").click(function(){
						var position = $(this).val();
		...
		var initControlStyle = function(){
			...
			var pieStyle = _style.pie;
			if(pieStyle){
				var pieSeries = pieStyle.series;
				if(pieSeries && pieSeries[0]) {
					...
					if(textStyle){
						...
						$("input[name='chart-pie-font-position-radio']").attr("disabled",true);
					}
					var textStylePosition = seriesStyle.series[0].itemStyle.normal.label.position;
					if(textStylePosition != null){
						var radioId = "chart-pie-font-position-radio-";
						...
						$("input[name='chart-pie-font-position-radio']").removeAttr("checked");
				...
				//是否显示图列
				var legendShow = (pieStyle.legend && pieStyle.legend.show === false) ? false :true;
				if(!legendShow){
					$("#chart-show-pie-legend-check").attr("checked",false);
				}else{
					$("#chart-show-pie-legend-check").attr("checked",true);
				}
			}//end if(pieStyle)

			var barStyle = _style.bar;
			if(barStyle){
				var barSeries = barStyle.series;
				...
				}//end if(barSeries && barSeries[0])

				//是否显示图列
				var legendShow = (barStyle.legend && barStyle.legend.show === false) ? false :true;
				if(!legendShow){
					//控制是否可以修改图例文字颜色
					$("#chart-show-legend-check").attr("checked",false);
