...
define(function(require, exports, module) {
	...
	var templateHtml = require("/http/js/module/style/highcharts/highcharts.3dpie.circle.single.html.tpl");
    ...
    var EChart3dPieStyle = (function(){
		...
		var eventBind = function(hanlder,callback){
			if(hanlder){
				...
				if(chartObj && objConfig){
                    ...
                    //饼图内环大小
                    $("#chart-inner-size-range").change(function(){
                        var innerSize = $(this).val();
                        $("#chart-inner-size-text").val(innerSize);
                        objConfig["option"] = _style = $.extend(true,_style,{
                            plotOptions: {
                                pie: {
                                    innerSize: innerSize
		...
		var initControlStyle = function(){
            ...
            if(_style.plotOptions) {
                ...
                var pieDepthStyleInnerSize = pieDepthStyle.plotOptions.pie.innerSize;
                if(pieDepthStyleInnerSize > -1) {
                    $("#chart-inner-size-range").attr("value",pieDepthStyleInnerSize).val(pieDepthStyleInnerSize);
                    $("#chart-inner-size-text").attr("value",pieDepthStyleInnerSize).val(pieDepthStyleInnerSize);
                }
            }
            if(_style.legend) {
