...
//			}
	...
	var SaveChart = (function(){
										...
										.json(function(re){
											...
											if(re.code == 0 && re.data){
												if(typeof callback == "function" && callback){
													...
													setTimeout(function(){
														window.location = "editantv?chart-id="+re.data.id;
													},2000);
												}
											}
										});
		...
		var getSubmitInfo = function(callback){
			...
			var styleUrl = "tool/js/module/style/antv/"+$chartTypeChart.val()+".style";
	...
	var Hanlder = (function() {
		...
		var setNameAndMemoAndImg = function(callback){
			...
			$.getJSON(configUrl,function(config){
				var name  = null;
				if(config && config.antv){
					name = config.antv[_chartTypeChart];
				}
				if(!name || name == null){
					name = "";
				}
				...
				$("<img style='max-width:100%' src='./image/antv/"+_chartTypeChart+".excel.png' />").appendTo($("#code-api"));
		...
		var bindEvent = function(){
			...
			$("#download_excel_template_btn").click(function(){
				var url = "excelTemplate/antv/"+_chartTypeChart+".template.xlsx";
				window.open(url,"_blank");
			});
		...
		var getTemplateData = function(callback){
			var url = SGIS.Config.BASE_MODULE_URL + "/js/module/antv/"
			...
			$.getJSON(url,function(data){
				if(_chartTypeChart=="echarts.parallel"){
					...
					//出图
					var jsUrl = "base/js/module/antv/"+_chartTypeChart;
					var styleUrl = "tool/js/module/style/antv/"+_chartTypeChart+".style";
