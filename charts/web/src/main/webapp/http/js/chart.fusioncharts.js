...
//			}
	...
	var SaveChart = (function(){
										...
										.json(function(re){
											...
											if(re.code == 0 && re.data){
												if(typeof callback == "function" && callback){
													...
													setTimeout(function(){
														window.location = "editfusioncharts?chart-id="+re.data.id;
													},2000);
												}
											}
										});
		...
		var getSubmitInfo = function(callback){
			...
			var styleUrl = "tool/js/module/style/fusioncharts/"+$chartTypeChart.val()+".style";
	...
	var Hanlder = (function() {
		...
		var setNameAndMemoAndImg = function(callback){
			...
			$.getJSON(configUrl,function(config){
				var name  = null;
				if(config && config.fusioncharts){
					name = config.fusioncharts[_chartTypeChart];
				}
				if(!name || name == null){
					name = "";
				}
				...
				$("<img style='max-width:100%' src='./image/fusioncharts/"+_chartTypeChart+".excel.png' />").appendTo($("#code-api"));
		...
		var bindEvent = function(){
			...
			$("#download_excel_template_btn").click(function(){
				var url = "excelTemplate/fusioncharts/"+_chartTypeChart+".template.xlsx";
				window.open(url,"_blank");
			});
		...
		var getTemplateData = function(callback){
			var url = SGIS.Config.BASE_MODULE_URL + "/js/module/fusioncharts/"
			...
			$.getJSON(url,function(data){
				if(_chartTypeChart=="echarts.parallel"){
					...
					//出图
					var jsUrl = "base/js/module/fusioncharts/"+_chartTypeChart;
					var styleUrl = "tool/js/module/style/fusioncharts/"+_chartTypeChart+".style";
